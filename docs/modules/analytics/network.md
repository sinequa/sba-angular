...
Nodes and edges are bundled in a container called a **dataset** ([`NetworkDataset`]({{site.baseurl}}analytics/classes/NetworkDataset.html) class). So providers essentially emit a dataset (whenever the data needs to be refreshed), and the component just *merges* the latest version of the datasets emitted by each provider.
