...
export class QueryWebService<T extends Results = Results> extends HttpService {
    ...
    public getResults(query: IQuery, auditEvents?: AuditEvents, queryIntentData?: QueryIntentData): Observable<T> {
        ...
        query = this.serializeDates(query);
    ...
    serializeDates(obj: any) {
      if(Array.isArray(obj)) {
        return obj.map(el => this.serializeDates(el));
      }
      if(typeof obj === 'object' && obj !== null) {
        if(obj instanceof Date) {
          return Utils.toSysDateStr(obj);
        }
        else {
          const newObj = {};
          for(let [k,v] of Object.entries(obj)) {
            newObj[k] = this.serializeDates(v);
          }
          return newObj;
        }
      }
