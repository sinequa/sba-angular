...
/**
 * Helper class to build selection expressions
 * @deprecated
 */
@Injectable({
  providedIn: "root",
...
export class ExprBuilder {
  ...
  private concatWithOperator(
    ...
    return values
      .map((v) => {
        ...
        if (v.display) {
          return `${ExprParser.escape(v.display)}:${ExprParser.escape(
            Utils.toSqlValue(v.value)
          )}`;
        }
  ...
  makeFieldExpr(
    ...
    return combineWithAnd
      ? this.makeAndExpr(field, items)
      : this.makeOrExpr(field, items);
  }
  ...
  makeAggregationExpr(
    ...
    if (aggregation.valuesAreExpressions) {
      ...
      return combineWithAnd
        ? this.concatAndExpr(exprs)
        : this.concatOrExpr(exprs);
    } else {
  ...
  private asValueItems(
    ...
    return items as ValueItem[]; // This works because ValueItem and AggregationItem share the value and display properties
  }
}
