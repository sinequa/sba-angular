...
export class SearchComponent implements OnInit, OnDestroy {
    ...
    // Dynamic display of labels facet
    public multiLabelsFacetIcon? = "fas fa-tags";
    public multiLabelsFacetTitle = "msg#facet.labels.title";
    ...
    // Whether the labels facet is multi or not
    public labelsFacetConfig: FacetConfig[];
    ...
    constructor(
        ...
        this._searchServiceSubscription = this.searchService.resultsStream.subscribe(
            (results) => {
                this.titleService.setTitle(
                    this.intlService.formatMessage("msg#search.pageTitle", {
    ...
    /**
     * Returns the configuration of the labels facet displayed in the facet-multi or facet-list component.
     */
    public get labelsFacet(): FacetConfig[] {
        if (this.loginService.complete && !this.labelsFacetConfig) {
            this.labelsFacetConfig = [];
            if (!!this.labelsService.publicLabelsField) {
                this.labelsFacetConfig.push({
                    name: this.labelsService.publicLabelsField,
                    title: "msg#labels.publicLabels",
                    type: "list",
            ...
            if (!!this.labelsService.privateLabelsField) {
                this.labelsFacetConfig.push({
                    name: this.labelsService.privateLabelsField,
                    title: "msg#labels.privateLabels",
                    type: "list",
    ...
    /**
     * Responds to a change of labels facet in the multi facet
     * @param facet
     */
    labelsFacetChanged(facet: FacetConfig) {
        if (!facet) {
            this.multiLabelsFacetTitle = "msg#facet.labels.title";
        } else {
            this.multiLabelsFacetTitle = facet.title;
        }
    }
    ...
    private isClickAction(event: Event): boolean {
        ...
        return (
            ...
            target.matches(
                "sq-result-selector *, .sq-result-title, sq-result-source *, sq-labels *"
            )
        );
    }
    ...
    toggleFilters() {
        ...
        if (this._showFilters) {
            // Close document if filters are displayed
            this.openedDoc = undefined;
        }
    }
