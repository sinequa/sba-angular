...
export class MoneyTimelineComponent extends AbstractFacet implements OnChanges,AfterViewInit {
    ...
    @Input() moneyColumn = "money"; // The "money" column stores an entity in the form "<CURRENCY> <NUMERAL>", for example "USD 69420"
    @Input() moneyAggregation = "Money"; // The "Money" aggregation is computed over the money column
    ...
    protected drawYAxis() {
        ...
        const yAxis = d3.axisLeft<number>(this.y)
            ...
            .tickFormat(this.yFormatter); //https://github.com/d3/d3-format
    ...
    filterDatum(datum: MoneyDatum) {
        const expr = this.exprBuilder.makeExpr(this.moneyColumn, datum.rawvalue, `${datum.currency} ${this.yFormatter(datum.value)}`)
