...
export const defaultChart = {
    "theme": "fusion",
    "highlightParentPieSlices": false, // prevent child pie slices from getting highlighted, when you hover over the parent slices
    "highlightChildPieSlices": true, // automatically highlight parent slices when you hover over the child pie slices
    "showPlotBorder": true,
    "piefillalpha": "60",
...
export class MultiLevelPieChart extends AbstractFacet implements OnChanges, OnDestroy, DoCheck {
    ...
    @Input() autoRotateLabels: boolean = false; // enable/disable rotation of plots label and use ellipses when overflow
    @Input() showToolTip: boolean = true;
    @Input() plottooltext: string = "$label, $value, $percentValue"; // configure the tooltip text of plots
    @Input() showValues: boolean = false; // Show/hide plots value next to labels
    @Input() showPercentValues: boolean = false; // if enabled along with "showValues", values of plots will be shown as percentages
