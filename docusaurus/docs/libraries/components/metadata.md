...
* `showEntityExtract` (default: `false`): Whether the entity extract should be displayed in a tooltip. Note that there are some prerequisites on the server like checking "return entity locations" in the [Query web service](../../guides/2-server-config#query-web-service-default-_query).
